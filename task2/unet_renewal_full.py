...
# unet= torch.load(r'/home/fisher/Peoples/suyeon/Paper/Unet/Unet012/Save_model/ES_epoch_9.pth')
unet= torch.load(r'/home/fisher/Peoples/suyeon/Paper/Unet/Unet012/Save_model/epoch_50.pth')
unet.to(torch.device('cuda:0'))
unet.eval()
...
for n in range(90,len(test_file)):
    ...
    #  print(f"filename = {new_name}")
     ...
     for m in range(len(true_bboxes)):
        ...
        with torch.no_grad():
            b = unet(newimg_trans.to(torch.device('cuda:0')))
        threshold=0.5
        outputs=  b.cpu().numpy()
        outputs = np.squeeze(outputs)
        outputs = (outputs >= threshold)
        output = outputs.transpose((1, 2, 0))
        ...
        # deeplab_output = (cc==1)
        unet_output = channel_1

        
        unet_output_img = Image.fromarray((unet_output).astype(np.uint8))
        unet_output_img = unet_output_img.resize(new_image_target_size)
        deeplab_arr= np.array(unet_output_img)
...
#         # 제목 리스트
        titles = ['Crop_Image','Crop_Ground','unet']
...
# # 제목 간 간격 조정
        ...
        plt.savefig(f'/home/fisher/Peoples/hseung/REAL_ARTICLE/unet_output_full/{new_name}_{m}.png')
